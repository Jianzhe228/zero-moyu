...
exports.ChapterDetector = void 0;
/**
 * 章节检测工具
 * 用于识别文本中的章节标题
 */
class ChapterDetector {
     ...
     * 检测文本中的所有章节
     */
    static detectChapters(content, pageSize) {
        const chapters = [];
        const foundChapters = new Set(); // 用于去重
     ...
     * 清理章节标题
     */
    static cleanChapterTitle(title) {
        // 移除多余的空白字符
     ...
     * 过滤有效的章节
     */
    static filterValidChapters(chapters) {
     ...
     * 检查是否为有效的章节标题
     */
    static isValidChapterTitle(title) {
        // 标题不能为空
     ...
     * 获取章节统计信息
     */
    static getChapterStats(chapters) {
        if (chapters.length === 0) {
            return {
                totalChapters: 0,
                averageLength: 0,
     ...
     * 根据当前位置查找所在章节
     */
    static findCurrentChapter(chapters, currentIndex) {
        if (chapters.length === 0) {
            return null;
        }
     ...
     * 查找下一章节
     */
    static findNextChapter(chapters, currentChapter) {
     ...
     * 查找上一章节
     */
    static findPreviousChapter(chapters, currentChapter) {
        const currentIndex = currentChapter.index;
        if (currentIndex > 0) {
            return chapters[currentIndex - 1];
        }
...
exports.ChapterDetector = ChapterDetector;
/**
 * 检测章节标题的正则表达式模式
 */
ChapterDetector.CHAPTER_PATTERNS = [
    // 中文数字章节
...
//# sourceMappingURL=ChapterDetector.js.map