...
export class FileService {
            ...
            for (const encoding of encodings) {
                try {
                    ...
                    if (encoding === 'gbk' || encoding === 'gb2312') {
                        ...
                        const content = buffer.toString(encoding as any);
    ...
    async selectMultipleFiles(): Promise<string[]> {
        ...
        if (fileUris) {
            return fileUris.map((uri: vscode.Uri) => uri.fsPath);
        }
     ...
     * 在文件夹中查找所有TXT文件
     */
    async findTxtFilesInFolder(folderPath: string): Promise<string[]> {
        const txtFiles: string[] = [];

        const findTxtFilesRecursive = (dir: string) => {
            try {
